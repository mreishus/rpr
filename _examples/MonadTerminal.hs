...
import           Control.Lens                        (ix, makeLenses, to, use,
                                                      view, (%=), (+=), (.=),
                                                      (^.), (^?!))
...
instance MonadTerminal MonadTerminalM where
  tmRender ts = do
    ...
    forM_ (toList unassignedColors) $ \c -> do
      ...
      when
        (nextColor > (fromIntegral maxColorID'))
        (error $ "more than " <> show maxColorID' <> " colors")
      cid <-
        liftCurses $
        C.newColorID
          (toCurses (c ^. cpForeground))
          (toCurses (c ^. cpBackground))
          (fromIntegral nextColor)
      tdNextColorId += 1
...
runTerminal ::
     Rect Int -- ^ The terminal viewport
  -> MonadTerminalM () -- ^ The actions to run inside a curses environment
  -> IO ()
runTerminal viewport a =
  C.runCurses $ do
    C.setEcho False
    w <-
      C.newWindow
        (viewport ^. rectHeight . to fromIntegral)
        (viewport ^. rectWidth . to fromIntegral)
        0
        0
